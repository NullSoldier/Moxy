...
namespace Moxy.ParticleSystems
{
		...
		: ParticleManager
	{
		...
		public void GenerateParticles (GameTime gameTime, ArcanaPlayer source, Vector2 Direction)
		{
			timeSinceLast += gameTime.ElapsedGameTime;
			if (timeSinceLast > particleDelay)
			{
				timeSinceLast = TimeSpan.Zero;
				var particle = new Particle (source.Location, ParticleTexture, 1f, 1f)
				{
					EndLocation = source.Location + (Direction * maxParticleRange),
